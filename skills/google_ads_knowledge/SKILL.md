---
name: google-ads-knowledge
description: Use this skill whenever planning, reviewing, or optimizing Google Ads campaigns for XPONext clients. Triggers when the user asks to create a Google Ads campaign, review ad performance, suggest keywords, set budgets, write ad copy, or advise on bidding strategies for local businesses. Contains real knowledge from the Google Ads certification course completed by Tim (XPONext).
---

# Google Ads Knowledge Base

> Wissen aus der Google Ads Zertifizierung (Skillshop) — praxisnah für lokale Betriebe aufbereitet.
...
2. [Kampagnenstruktur & Setup](#2-kampagnenstruktur--setup)
3. [Ad Rank & Auktion](#3-ad-rank--auktion)
4. [Keyword-Strategie](#4-keyword-strategie)
5. [Responsive Search Ads (RSAs)](#5-responsive-search-ads-rsas)
6. [Bid Strategies](#6-bid-strategies)
7. [Performance Planner](#7-performance-planner)

---

## 1. AI-powered Search Ads

Die drei KI-Hebel im Google Suchnetzwerk — immer zusammen einsetzen für maximale Performance:

**Broad Match** — Keywords weit auslegen, KI versteht Suchintention. Spart Pflege von langen Keyword-Listen. Gut für lokale Betriebe, die viele Varianten nicht selbst vorhersehen können.

**Smart Bidding** — Automatisches Gebotsmanagement auf Basis von Nutzersignalen (Uhrzeit, Gerät, Standort, Verhalten). Optimiert auf ROAS oder CPA. Für Kundenkampagnen: immer Conversion-Tracking einrichten, sonst hat Smart Bidding keine Datenbasis.

**Responsive Search Ads (RSAs)** — Bis zu 15 Headlines + 4 Descriptions eingeben, Google testet Kombinationen automatisch. Für Kunden: möglichst viele hochwertige Assets liefern (Headlines, Sitelinks, Bilder, Standort), damit die KI mehr Varianten testen kann.

> Alle drei zusammen: durchschnittlich **+20% Conversions** bei gleichem CPA (Google-Studie).

**Einsatzgebiet Search:** Nur wenn Nutzer aktiv suchen — Pull-Kanal, keine Awareness-Kampagnen. Ideal für lokale Dienstleister mit klarer Kaufabsicht beim Nutzer (Handwerker, Praxen, KFZ).

---

## 2. Kampagnenstruktur & Setup

**Kontostruktur:** Manager Account → Account (pro Kunde) → Kampagne (eigenes Budget) → Ad Group (ein Thema).

Als Agentur: Manager Account anlegen, dann pro Kunde ein eigenes Account darunter verwalten.

**Kampagnenaufbau für lokale Kunden:**
- Ziel meist **Leads** (Anruf, Formular) oder **Sales**
- Netzwerk: nur Search Network, Display deaktivieren — Budget gezielt in der Suche halten
...
**Alternativen ohne Keywords:**
- DSAs — sinnvoll wenn Kunde viele Leistungsseiten hat
- Performance Max — für Kunden mit breitem Angebot und Conversion-Tracking

---

## 3. Ad Rank & Auktion

Google vergibt Anzeigenpositionen per Auktion — nicht nach höchstem Gebot, sondern nach **Ad Rank**.

**Ad Rank = Gebot × Qualität × Kontext.** Wer eine bessere Anzeige hat, zahlt weniger pro Klick und steht trotzdem oben.

**6 Faktoren:** Gebot, Qualitätsschwelle (Threshold), Suchkontext (Gerät, Ort, Uhrzeit), Assets, Anzeigenqualität (Quality Score), Wettbewerb.

**Quality Score — 3 Teilfaktoren:**
- **Expected CTR** — wird die Anzeige geklickt?
- **Landing Page Experience** — ist die Zielseite relevant und transparent?
- **Ad Relevance** — passt die Anzeige zum Suchbegriff?
...
**Match Types:**
- `[Exact]` — engste Kontrolle, geringste Reichweite
- `"Phrase"` — Kernbegriff muss in Suchanfrage enthalten sein
- `Broad` (kein Symbol) — KI findet relevante Varianten automatisch; Standard-Empfehlung

Nicht denselben Begriff in allen drei Match Types gleichzeitig einsetzen — Broad Match deckt die anderen ab.

**Für Kundenkampagnen:**
- Mit Broad Match + Smart Bidding starten
- Vor der Skalierung Broad Match testen: One-Click-Experiment über Recommendations-Seite oder Custom Experiment. Mindestens 3–4 Wochen laufen lassen, Budget während des Tests nicht deckeln
- Ziel für aussagekräftige Ergebnisse: 30 Conversions im Testzeitraum

**Ad Groups:** Pro Thema eine Ad Group, Keywords müssen zur Landing Page passen. Thematische Bündelung hilft der KI, die richtige Anzeige auszuspielen.

**Keyword-Pflege:**
- **Keyword Planner** → Suchvolumen und neue Ideen
- **Search Terms Report** → sehen, welche echten Suchanfragen Anzeigen ausgelöst haben
- **Negative Keywords** → irrelevante Suchanfragen ausschließen, aber sparsam einsetzen — zu viele blockieren potenzielle Reichweite
- **Optimization Score** → Empfehlungen für Duplikate, Konflikte, neue Keywords

---

## 5. Responsive Search Ads (RSAs)

15% aller Suchanfragen täglich sind neu — RSAs fangen auch diese ab, indem Google automatisch die beste Kombination aus bereitgestellten Assets für jeden Nutzer ausspielt.

**Für Kundenkampagnen — Mindestanforderungen:**
- Mind. 10 Headlines, mind. 3 Descriptions
- Mind. 3 Keywords in Headlines einbauen
...
- Callouts: mind. 6 pro Kampagne, kurz und spezifisch
- Structured Snippets: Header-Format "Leistung: Wert1, Wert2"
- Automatically Created Assets auf Kampagnenebene aktivieren → KI generiert passende Headlines aus Landing Page
...
**Welche Strategie für welches Kundenziel:**
- Mehr Traffic → Maximize Clicks
- Mehr Sichtbarkeit / Markenaufbau → Target Impression Share
- Mehr Anfragen/Leads → Maximize Conversions (mit Ziel-CPA, sobald genug Conversion-Daten vorhanden sind)
- Mehr Umsatz/Wert pro Abschluss → Maximize Conversion Value (mit Ziel-ROAS)
...
2. **Tracking eingerichtet, noch wenig Daten:** Maximize Conversions (zunächst ohne CPA-Ziel) — sammelt Conversion-Daten, ohne das Volumen durch ein zu enges Effizienzziel zu begrenzen
3. **Genug Daten vorhanden (Richtwert 15 Conversions/30 Tage):** Wechsel zu Target CPA (Lead-Fokus) oder Target ROAS (Sales/Wert-Fokus) — Ziel realistisch an der historischen Performance ausrichten, nicht zu ambitioniert ansetzen
...
**Performance hängt nie nur an der Bid Strategy — weitere Hebel für Kundenkampagnen:**
- **Keywords/Reichweite:** neue Keywords ergänzen, breitere Match Types (Richtung Broad Match), DSAs für automatische Abdeckung neuer Suchanfragen
- **Audiences (Zielgruppenlisten):** Customer Match / Remarketing Lists for Search Ads (RLSA) — Past Visitors erneut ansprechen, ähnliche neue Kunden finden, bestehende Kunden zu Folgegeschäft bewegen. Besonders für Kunden mit vorhandener Kontakt-/Kaufhistorie ein unterschätzter Hebel
- **Creative:** Ad Extensions (zusätzliche Infos/Lenkung auf Zielseiten), Ad Customizers (Standort/Countdown für hyper-relevante Anzeigen), Keyword Insertion (Suchwort des Nutzers im Anzeigentext spiegeln → höhere wahrgenommene Relevanz)
...
- **Countdown** → Zeitdruck erzeugen bei Aktionen/Events
- **Keyword Insertion** → Suchbegriff des Nutzers direkt im Anzeigentext spiegeln → höhere Relevanz → besserer Quality Score
- Setup: Business Data → Customizer erstellen → Werte eintragen → Code in RSA einfügen. Ohne Werte keine Ausspielung.
